...
const ViewOrder = () => {
  ...
  // Check if orderState is an array and has a length greater than 0
  if (Array.isArray(orderState) && orderState.length > 0) {
    const productsArray = orderState[0].products;
    console.log(productsArray);
  } else {
    console.log("Order state is undefined or empty");
  }
  const data1 = [];
  ...
  for (let i = 0; i < orderState.length; i++) {
    ...
    if (order && order.products && order.products.length > 0) {
      const productEntry = order.products[0];
